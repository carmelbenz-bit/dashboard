...
export function CaseCard({ caseData, onEditTask, onDeleteTask, onCompleteTask, onAddTask, onDeleteCase, onAddHearing, onAddMeeting, onEditMeeting, onDeleteMeeting, onCompleteMeeting, onEditCase }: CaseCardProps) {
  ...
  return (
    <div className="bg-card rounded-xl border border-border overflow-hidden shadow-sm">
      ...
      <div className="bg-sidebar p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3
                className="text-lg font-semibold text-sidebar-foreground flex items-center gap-2 cursor-pointer hover:text-primary transition-colors"
                onClick={onEditCase}
              >
              ...
              <Popover>
                ...
                >
                  <div className="space-y-1">
                    <div className="text-sm font-medium text-slate-600 px-2 py-1">
                      קבצים מצורפים
                    </div>
      ...
      {isExpanded && (
        <div className="p-5">
          <div className="grid grid-cols-12 gap-3 px-4 py-3 bg-slate-50 rounded-t-lg border border-b-0 border-slate-200 text-xs font-semibold text-slate-500 uppercase tracking-wide">
            ...
            {caseData.tasks.map((task, index) => {
              ...
              return (
                <div
                  key={task.id}
                  className={cn(
                    "grid grid-cols-12 gap-3 px-4 py-4 items-center transition-colors",
                ...
                >
                  ...
                  <div className="col-span-2 flex justify-center">
                    {assigneeTag ? (
                      <AssigneeBadge assignee={assigneeTag as "שלי" | "הצד השני"} />
                    ) : (
                  ...
                  <div className="col-span-2 text-center">
                    <span className={cn(
                      "text-sm",
                      task.dueDate ? "text-foreground" : "text-muted-foreground/50"
                    )}>
                      {task.dueDate || "—"}
                    </span>
                  </div>
            ...
            <div className="mt-4 space-y-2">
              {caseData.meetings.map((meeting) => {
                ...
                return (
                  <div
                    key={meeting.id}
                    className="grid grid-cols-12 gap-3 px-4 py-3 bg-purple-50 border border-purple-100 rounded-lg items-center"
                  >
                    <div className="col-span-4 flex items-center gap-3">
                      <button
                      ...
                      <div className={cn(
                        "w-1 h-10 rounded-full flex-shrink-0",
                        meeting.completed ? "bg-slate-300" : "bg-purple-400"
                      )} />
                      <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={cn(
                            "text-sm font-medium",
                            meeting.completed ? "text-slate-400 line-through" : "text-slate-700"
                          )}>{meeting.title}</span>
                        </div>
                        {meeting.notes && (
                          <span className="text-xs text-muted-foreground mr-1">{meeting.notes}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="col-span-2 flex justify-center gap-1">
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 w-8 p-0 text-slate-500 hover:text-purple-600 hover:bg-purple-100"
