...
async function signVapidJwt(audience: string): Promise<string> {
  ...
  // Import as raw JWK with the d scalar
...
async function sendPush(subscription: { endpoint: string; keys: { p256dh: string; auth: string } }, payload: object) {
  ...
  // For simplicity we send unencrypted — browsers require encryption,
  // so we use the built-in Web Crypto to encrypt.
...
Deno.serve(async () => {
      ...
      for (const item of items) {
        ...
        for (const reminder of reminders) {
          ...
          if (triggerAt >= windowStart && triggerAt <= now) {
            // This reminder is due — send push to all user's subscriptions
            ...
            updates.push((async () => {
              ...
              await Promise.allSettled(
                (subs ?? []).map((s) => sendPush(s.subscription as { endpoint: string; keys: { p256dh: string; auth: string } }, { title, body, url: "/dashboard", tag: reminder.id }))
              )
            })())
  ...
  return new Response(JSON.stringify({ ok: true, time: now.toISOString() }), { status: 200 })
})
